...
|---|---|---|---|---|
| 0 | Bootstrap | `fire_campaign.sh` → `campaign_config.py` (validates `campaign.json` against the one schema, materialises it into the per-file layout in a new auto-suffixed `runs/<slug>/`) | `runs/<slug>/` | fixture missing a required piece |
...
**Fixture** (`templates/<base>/`, see `templates/README.md`): `campaign.json` (every knob: channels, draft_mode, vertical, countries, enrich_cap, target_roles, fetch_pages, linkedin, li_max_*, sourcing, notes) + `icp.yaml` + `pitch.json` + `places.txt` [+ `qualify.json`]. **Run control files** (materialised from it by `campaign_config.py`, so no stage reads `campaign.json`): `sourcing.json`, `draft_mode.txt`, `channels.json`, `countries.txt`, `vertical.txt`, `target_roles.txt`, `fetch_pages.txt`, `enrich_cap.txt`, `linkedin.json`, `li_max_per_company.txt`, `li_max_people.txt` — each only when non-default — plus the content files copied verbatim.
