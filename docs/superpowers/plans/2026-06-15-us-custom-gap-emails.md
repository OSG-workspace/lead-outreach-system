...
```bash
cd <home>/Desktop/lead-outreach-system/project && source tools/venv/bin/activate
```
...
Create `<home>/Desktop/lead-outreach-system/.claude/agents/gap-finder.md`:
...
grep -c "Reyes Family Law" vault/lead-outreach/voice-us.md
...
Overwrite `<home>/Desktop/lead-outreach-system/.claude/agents/gap-writer.md`:
...
if [ -f "$RUN/draft_mode.txt" ] && [ "$(head -1 "$RUN/draft_mode.txt" | tr -d ' \n')" = "custom" ]; then
    for need in .claude/agents/gap-finder.md .claude/agents/gap-writer.md \
        ...
        [ -f "<home>/Desktop/lead-outreach-system/$need" ] \
